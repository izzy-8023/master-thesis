...
class psne(nn.Module):
    ...
    def __init__(self, n_dim, n_node):
        ...
        self.init_embedding()
        self.pmi = nn.Linear(n_dim, 1, True)
        self.sign_prob = nn.Linear(n_dim, 1, True)
        self.sm = nn.Sigmoid()

    def forward(self, s, t):
        emb_u = self.embedding_u(s)
        emb_v = self.embedding_v(t)
        edge = torch.mul(emb_u, emb_v)
        out1 = self.pmi(edge)
        out2 = self.sign_prob(edge)
        out2 = self.sm(out2)
...
def main(data = 'wkr', des = 'cv', des2 = '0', n_dim = 20, beta = 0.01, lr = 1e-1, lam = 5e-5, use_gpu = False, seed=1, 
    max_itr = 200,train=1, resultdir = 'result_fsne/'):
    print('lSNE: k:%d, data:%s, des:%s, des2:%s,beta:%.4f,lr:%f,lam:%.6f' % (n_dim, data, des, des2, beta,lr,lam))
    ...
    resultfile = open(resultdir+'l.%s.%s.%s.%d.b%.4f,r%.2f,l%.6f.report.txt'  % (data, des, des2,n_dim, beta,lr,lam), 'w')
    ...
    if train:
        ...
        deg_out = deg_matrix.sum(0).getA1().astype(int)
        deg_in = deg_matrix.sum(1).getA1().astype(int)
        deg_out[deg_out == 0] = 1
        deg_in[deg_in == 0] = 1
        ...
        model.save_embedding('/Users/izzy/Desktop/Signed-Network-Embedding-master/result_fsne')
        # test
        ...
        np.savetxt(resultdir+'f.%s.%s.%s.k%d.b%.4f,r%.2f,l%.6f.B.txt'  % (data, des, des2,n_dim, beta,lr,lam),B)
        np.savetxt(resultdir+'f.%s.%s.%s.k%d.b%.4f,r%.2f,l%.6f.W.txt'  % (data, des, des2,n_dim, beta,lr,lam),W)
    else:
        B = np.loadtxt(resultdir+'f.%s.%s.%s.k%d.b%.4f,r%.2f,l%.6f.B.txt'  % (data, des, des2,n_dim, beta,lr,lam))
        W = np.loadtxt(resultdir+'f.%s.%s.%s.k%d.b%.4f,r%.2f,l%.6f.W.txt'  % (data, des, des2,n_dim, beta,lr,lam))
    
    if des != 'all':
        if train:
            # pmi, sign_prob = model(test_s_list, test_t_list)
            _, f1, ruc = test_sign_pred(model, test_s_list[n_val:], test_t_list[n_val:], test_edges[n_val:,2], use_cuda = use_gpu)
...
if __name__=='__main__':
    ...
    parser.add_argument('--dir', type=str, default = 'result_fsne/')
